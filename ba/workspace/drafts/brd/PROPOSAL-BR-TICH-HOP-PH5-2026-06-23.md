...
| Mã BR | Phát biểu yêu cầu | Loại | Ưu tiên | Nguồn | Trạng thái KS | Triển khai sheet v77 | Phụ thuộc |
|---|---|---|:---:|---|:---:|:---:|---|
| **BR-567** | Hệ thống TOSS phải lấy **Crew Assignment** (thông tin tổ bay) từ AVES và gửi sang OPS++ theo cơ chế: (a) **quét snapshot lịch tháng kế tiếp vào 02:00 ngày 26 hàng tháng**; (b) đếm **số chuyến bay** có dữ liệu tổ bay trong tháng kế tiếp, nếu **dưới 1000 chuyến** thì coi là chưa có dữ liệu và **sinh cảnh báo qua email** cho danh sách người quản lý đã cấu hình (kế thừa BR-537) — *(đơn vị = số chuyến bay, BA Lead chốt 23/06)*; (c) **cơ chế retry trong cùng ngày 26** nếu lần quét initial chưa đủ dữ liệu (mốc giờ cụ thể chưa chốt); (d) ngoài snapshot tháng, có **cơ chế quét cập nhật theo rules** (mô hình tương tự Amadeus) khi nguồn AVES có thay đổi giữa kỳ. | Functional | Must | Khảo sát 23/06 §II.9; sheet v77 dòng 12 | PARTIAL | Đã có | **D-11 đã chốt (số chuyến bay).** Còn mở: mức ngưỡng 1000 có cấu hình được không; mốc giờ retry; danh sách email nhận cảnh báo |
