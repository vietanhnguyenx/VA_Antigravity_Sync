...
>
> **Bổ sung v0.2:** cập nhật theo Biên bản họp **MM-20260609** (Workshop Thiết kế Function List / Menu + rà soát YCKT, 09/06/2026 — REF-08): bổ sung BR-TOSS-031…040, cấu trúc Function List (§4.1.1), thuật ngữ mới (§1.4), câu hỏi mở (§10.9). **Lưu ý:** biên bản lập từ transcript ASR chất lượng thấp; người dự họp và người quyết định các DEC-09-XX **đang chờ xác nhận** — các BR mới giữ trạng thái Draft cho tới khi được xác nhận.
...
| REF-08 | MM-20260609 — Biên bản họp Thiết kế Function List / Menu + rà soát YCKT (09/06/2026) | `ba/workspace/input/Customer_docs/meeting-notes/MM-20260609-thiet-ke-function-list-YCKT-TOSS.md` *(tổng hợp từ transcript ASR chất lượng thấp; người dự/người quyết định chờ xác nhận)* |
...
| GENDEC (CD/GDPM) | Tờ khai chung — danh sách tổ bay (phi công, tiếp viên, nhân viên mặt đất lên tàu) | General Declaration *(làm rõ tại họp 09/06 — REF-08 §3)* |
...
| Dry Lease | Thuê khô — thuê tàu bay không kèm tổ bay/dịch vụ của bên cho thuê *(REF-08 §5)* | — |
...
| PP-07 | Danh mục dịch vụ/sự cố sân bay hiện quản lý **thủ công**, chưa có phần mềm, **không trích xuất/tổng hợp được** | REF-08 §6 |
...
- **3 cụm quản trị tách riêng (level 0):** **System Admin** · **Data Monitoring** · **Data Maintenance** (DBM/danh mục). Đã **chốt tách Data Maintenance thành menu riêng** vì nhiều nội dung liên quan dữ liệu/danh mục *[DEC-09-01]*.
- **Danh mục (master data) vs nghiệp vụ:** danh mục là dữ liệu cơ sở (cố định hoặc được quản lý) phục vụ nghiệp vụ phía trên; đồng bộ tự động hàng ngày; giá trị chính là **phục vụ cảnh báo** (ví dụ cảnh báo MEL) cho tầng nghiệp vụ *[REF-08 §1]*.

> *Ánh xạ cụm menu ↔ 5 phân hệ (§4.1) sẽ được chi tiết hóa trong SRS. Người quyết định DEC-09-01 đang chờ xác nhận (REF-08 §Câu hỏi).*
...
| SH-12 | Nhân viên Kiểm soát tải (Load Control) | VNA | P1 | Nhập Load & Weight, cung cấp tài liệu chuyến bay, lưu NOTOC/Cargo Manifest | REF-08 §3 |
| SH-13 | Đại diện khai thác tại sân bay (Station Manager) | VNA | P2 | Checklist an ninh (security), xem/tải tài liệu chuyến bay | REF-08 §4 |
...
> **Quy tắc:** Mỗi BR mô tả **NHU CẦU NGHIỆP VỤ** (TẠI SAO làm), KHÔNG mô tả chức năng kỹ thuật (LÀM GÌ — sẽ ở SRS). Ưu tiên theo **MoSCoW**: **M** = Must Have, **S** = Should Have, **C** = Could Have, **W** = Won't Have (giai đoạn này). BR bổ sung từ họp 09/06 (REF-08) **chưa được xếp ưu tiên** — chờ BA Lead/PO quyết định.
...
| **BR-TOSS-031** | Nghiệp vụ Kiểm soát tải (Flight Load Control) | VNA cần nhóm chức năng Flight Load Control gồm 3 nhóm: (1) **nhập dữ liệu Load & Weight** (load and balance); (2) **cung cấp tài liệu** chuyến bay; (3) **lưu NOTOC** (2 loại: hành khách + cargo) và **Cargo Manifest (CM)** | REF-08 §3 | *(chưa xếp — chờ PO)* |
| **BR-TOSS-032** | Upload tài liệu hàng loạt (bulk) cho nhiều chuyến bay | VNA cần khả năng **upload nhiều tài liệu cho nhiều chuyến** trong một thao tác, tách thành **tab riêng**; hệ thống **tự động gắn file vào chuyến** theo quy ước tên file (`VN<flight> <ngày> <version> <loại>`); loại tài liệu: Loadsheet, CD/GDPM, PM, NOTOC. Vẫn **giữ chức năng upload thủ công** từng chuyến | REF-08 §3 + DEC-09-03 | *(chưa xếp — chờ PO)* |
| **BR-TOSS-033** | Chính sách lưu trữ tài liệu theo nguồn gốc (Retention) | Tài liệu **sinh ra từ TOSS** (GDPM, NOTOC…) → TOSS lưu trữ. Tài liệu **từ MO/Lido** (OFP/OSP, FP, NOTAM, thời tiết — thứ TOSS không sở hữu) → chỉ **kéo về khi người dùng download**, lưu **tối đa 3 ngày sau khi hạ cánh** rồi job tự động xóa (retention theo từng cục dữ liệu) | REF-08 §4 + DEC-09-04 *(khớp MM-20260608 §14)* | *(chưa xếp — chờ PO)* |
| **BR-TOSS-034** | Nghiệp vụ Đại diện khai thác tại sân bay (Station Manager) | Đối tượng Station Manager cần **menu riêng** (đối tượng nghiệp vụ nặng), chủ yếu gồm: **checklist an ninh (security)** + **xem/tải tài liệu** chuyến bay | REF-08 §4 | *(chưa xếp — chờ PO)* |
...
| **BR-TOSS-013** | Đánh giá hiệu suất đúng giờ (OTP/OSP) & biến động lịch bay | Lãnh đạo cần **phân tích tỷ lệ chuyến bay đúng giờ**, thống kê chậm chuyến, chuyển hướng, đổi giờ/tàu/cấu hình/code/hủy chuyến. **OTP/OSP tính tự động** — quy ước: cất cánh **≤ 14 phút** sau giờ dự kiến = đúng giờ; trễ thêm 1 phút = chậm chuyến (delay) | REF-03 / 03 §Báo cáo OTP + REF-08 §7 | M |
...
| **BR-TOSS-035** | Giám sát chất lượng dữ liệu (Data Monitoring) | VNA cần **báo cáo độ phủ & độ chính xác dữ liệu** (ví dụ: tỷ lệ chuyến bay có điện ACARS theo đội tàu — A321 ~50%, 787 ~70–80%, A380 ~90%) — **tách màn hình riêng**, KHÔNG gộp vào báo cáo nghiệp vụ và KHÔNG phải giám sát vận hành server | REF-08 §7 + DEC-09-05 | *(chưa xếp — chờ PO)* |

> **Ghi chú phân tầng người dùng báo cáo** *(REF-08 §7)*: **trang đầu (cấp xem nhanh)** quan tâm ngày/khách/thời tiết hiện tại; **người đánh giá** cần **thống kê dài hạn (1–2 năm)** trên số lớn — sẽ chi tiết hóa ở SRS.
...
| **BR-TOSS-036** | Quản lý vòng đời & lịch sử tàu bay theo giai đoạn hiệu lực | VNA cần quản lý **lịch sử vòng đời tàu bay** theo registration: tàu **vào/ra đội bay nhiều lần** (thuê), **đổi cấu hình khoang (reconfig)**, loại tàu từng khai thác (vd 777/787) — mỗi thay đổi có **ngày hiệu lực/hết hiệu lực**. Quản lý theo **hình thức sở hữu: Sở hữu / Thuê khô (Dry Lease) / Thuê ướt (Wet Lease)** (thuê khô = thuê tàu, đội ngũ của hãng; thuê ướt = thuê nguyên tàu + phi công/tiếp viên/dịch vụ của bên cho thuê; có trường hợp "mix" — vd chỉ cover dịch vụ nhiên liệu) | REF-08 §5 + DEC-09-06 | *(chưa xếp — chờ PO)* |
| **BR-TOSS-037** | Thuộc tính tàu bay & phân nhóm đội tàu (Category) phục vụ báo cáo | VNA cần quản lý **thuộc tính tàu bay**: trọng tải, cấu hình khoang (30DC/30DL Deluxe/30DY), dung tích thùng nhiên liệu (fuel tank — 2 tank), thùng nước (water tank), khoang hàng (compartment), MEL/CDL, ICAO/IATA designator. Cần **nhóm/Category đội tàu** (vd 320+321 = 1 category; widebody/narrowbody; A350/787/777/ATR) — **mỗi loại báo cáo dùng cách chia nhóm khác nhau** (fuel flow / taxi time / định mức nhiên liệu) | REF-08 §5 + DEC-09-06 | *(chưa xếp — chờ PO)* |
| **BR-TOSS-038** | Hệ số quy đổi đơn vị nhiên liệu ACARS theo giai đoạn | Điện văn ACARS có thể ghi nhiên liệu bằng **tấn hoặc đơn vị khác** → VNA cần **hệ số quy đổi về kg** có **hiệu lực theo giai đoạn** (lưu lịch sử), vì cùng một tàu có thể đổi đơn vị khi cập nhật hệ thống | REF-08 §5 | *(chưa xếp — chờ PO)* |
| **BR-TOSS-039** | Danh mục dịch vụ sân bay & cảnh báo năng lực phục vụ | VNA cần **danh mục master dịch vụ/sự cố sân bay** (hiện quản lý thủ công, không trích xuất được): ví dụ khai báo sân bay **không hỗ trợ tàu hỏng APU** (không có xe khởi động/GPU) → khi tàu **hỏng APU** được điều đến sân đó, hệ thống phải **cảnh báo + yêu cầu đổi tàu** | REF-08 §6 | *(chưa xếp — chờ PO)* |
...
| **BR-TOSS-040** | Phân quyền theo chức năng & màn hình dùng chung ẩn/hiện theo quyền | VNA cần mô hình **phân quyền theo CHỨC NĂNG** (không theo dữ liệu); riêng **dữ liệu chuyến bay phân theo "main bay" (flight base)**. Nhiều đối tượng **dùng chung một màn hình** — **cột và nút ẩn/hiện theo quyền** (vd 50 cột: ~10 cột trạng thái ai cũng thấy, 10 cột NOTOC, 10 cột Fly Release, 10 cột Load & Weight); tách quyền `view` với `upload`/`review`. Người dùng được **tùy biến (customize) cột hiển thị, lưu 1 lần** | REF-08 §2 + DEC-09-02 | *(chưa xếp — chờ PO)* |
...
| ASM-07 | File tài liệu từ hệ thống nguồn (GDPM…) có **quy ước đặt tên ổn định** đủ để auto-gắn vào chuyến bay (BR-TOSS-032) — sẽ xác minh tại buổi khảo sát luồng GDPM | REF-08 §3 + Action Item 1 |
...
### 10.9 Câu hỏi phát sinh từ họp 09/06/2026 (REF-08)
- Q-19: Bulk upload phân biệt tài liệu theo **tên file hay content**? — chốt sau **buổi khảo sát luồng GDPM** (định dạng/quy ước đặt tên file nguồn xuất ra) *(Action Item 1)*.
- Q-20: **Đơn vị nhiên liệu trong điện văn ACARS** theo từng giai đoạn, từng tàu — cần khảo sát nguồn để lập bảng hệ số quy đổi *(Action Item 2)*.
- Q-21: **Nguồn dữ liệu danh mục dịch vụ sân bay** (hiện quản lý thủ công, không trích xuất được) — ai cung cấp, định dạng nào? *(Action Item 3)*.
- Q-22: **Định nghĩa Category/nhóm đội tàu** cho từng loại báo cáo (fuel flow / taxi time / định mức nhiên liệu) *(Action Item 4)*.
- Q-23: **Định danh người dự họp 09/06 & người quyết định** các DEC-09-01…06 — cần xác nhận trước khi nâng trạng thái các BR-TOSS-031…040.
- Q-24: **Ưu tiên MoSCoW** cho các BR-TOSS-031…040 — chờ BA Lead/PO xếp loại.
...
| BR-TOSS-031…034 | 2. Quản lý tài liệu chuyến bay (Load Control, bulk upload, retention, Station Manager) | `Customer_docs/meeting-notes/MM-20260609-thiet-ke-function-list-YCKT-TOSS.md` §3–§4 |
| BR-TOSS-035 | 3. Báo cáo & Tối ưu khai thác (Data Monitoring) | `MM-20260609` §7 + DEC-09-05 |
| BR-TOSS-036…039 | 4. Quản lý danh mục (Tàu bay, ACARS, Sân bay) | `MM-20260609` §5–§6 + DEC-09-06 |
| BR-TOSS-040 | 5. Quản trị hệ thống (Phân quyền theo chức năng) | `MM-20260609` §2 + DEC-09-02 |

---

*BRD-TOSS-001 v0.2 — Bản nháp do business-analyst agent biên soạn theo phương pháp top-down, tuân thủ nguyên tắc CLAUDE.md §0 (phân rã + tái cấu trúc theo nguồn, không suy diễn). Mọi yêu cầu được truy vết về tài liệu nguồn. Các nội dung thiếu được đánh dấu `(chưa có — cần làm rõ)` và tập hợp tại §10.*

*Lịch sử phiên bản:*
- *v0.2 (2026-06-10): Bổ sung theo MM-20260609 (REF-08) — thêm BR-TOSS-031…040 (Flight Load Control, bulk upload, retention, Station Manager, Data Monitoring, vòng đời tàu bay & sở hữu/thuê, Category đội tàu, quy đổi ACARS, danh mục dịch vụ sân bay, phân quyền theo chức năng); §4.1.1 cấu trúc Function List; SH-12/SH-13; PP-07; ASM-07; DEP-08; thuật ngữ GENDEC/CM/ACARS/APU/Dry-Wet Lease/OSP; cập nhật BR-TOSS-013 (quy ước OTP/OSP ≤14 phút); §10.9 Q-19…Q-24. Các BR mới chưa xếp ưu tiên, chờ xác nhận người quyết định.*
- *v0.1 (2026-06-04): Bản nháp đầu tiên từ nguồn VNAOCC_TOSS_Đề xuất giải pháp kỹ thuật_v0.3.pdf.*
