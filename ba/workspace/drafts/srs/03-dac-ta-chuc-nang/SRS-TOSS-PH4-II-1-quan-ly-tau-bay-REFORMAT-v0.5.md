...
| 4 | Valid To | Textview (xem) / DateInput (sửa) | `valid_to` | ❌ Không | Ngày kết thúc hiệu lực, format `dd/mm/yyyy`. Khi sửa phải `> Valid From` (không cho trùng). Cho phép ngày quá khứ (CH-02) |
...
| 8 | MTOW | NumberInput | `mtow` | Trọng lượng cất cánh tối đa, đơn vị `kg`. Validate: số `> 0` và `≥ Max Ramp Weight`. **CH-03: REJECT nếu MTOW < Max Ramp Weight** |
...
| Tham số kỹ thuật (8 trường) | ❌ Không | Validate số `≥ 0` hoặc `> 0`; ràng buộc `Basic Weight ≤ Max Ramp Weight ≤ MTOW` (CH-03: REJECT nếu MTOW < Max Ramp Weight) |
...
| :---: | :---- | :---- | :---- | :---- |
| US-001 | Danh sách tàu bay - View và export | Người dùng xem danh sách tàu bay, hỗ trợ lọc, phân trang, xuất Excel | **Given:** User login + quyền View Aircraft; **When:** Click module Aircraft Fleet; **Then:** Danh sách hiển thị, hỗ trợ lọc AC Subtype/ACreg/Category/Status, phân trang 10-100 dòng/trang, nút Export Excel ghi tên file theo CH-10 | Mục 1, CH-10 |
...
| US-003 | Xem chi tiết Tab 1 - Thông tin chung (xem) | Người dùng xem đầy đủ 9 trường thông tin chung, locked fields có icon | **Given:** User click vào dòng danh sách; **When:** Mở Tab 1; **Then:** Hiển thị 9 trường, AC Subtype & Valid From có icon khoá (CH-01), không thể edit | Mục 2.1, CH-01 |
| US-004 | Sửa Tab 1 - Thông tin chung | Người dùng sửa các field editable trong Tab 1, validate CH-02/CH-04, save/cancel | **Given:** User có quyền Edit Aircraft > General Info; **When:** Click Edit → sửa dữ liệu → Save; **Then:** Validate (Aircraft Type Name bắt buộc, Valid To > Valid From CH-02, Owner logic CH-04) → Audit log ghi IP/UA → Toast "Saved" | Mục 2.1, CH-02/CH-04/CH-08 |
...
| US-006 | Sửa Tab 2 - Block tham số kỹ thuật | Người dùng sửa 8 tham số, validate weight constraint (CH-03: REJECT nếu MTOW sai) | **Given:** User nhấn Edit block tham số; **When:** Sửa MTOW < Max Ramp Weight → Save; **Then:** Toast "REJECT — MTOW must be ≥ Max Ramp Weight" (CH-03), không lưu | Mục 2.2.1, CH-03/CH-08 |
| US-007 | Sửa Tab 2 - Block cấu hình khoang | Người dùng điều chỉnh Cabin A/B/C (max 3), validate tổng cabin cap (CH-06) | **Given:** User sửa Cabin A/B/C; **When:** Tổng > cap loại tàu → Save; **Then:** Toast "REJECT — total cabins exceed aircraft capacity" (CH-06), không lưu | Mục 2.2.2, CH-06/CH-08 |
