...
parent_document: "BRD-TOSS-PH4-quan-ly-danh-muc-v0.5.md"
...
> **Phiên bản này (v0.4 — 2026-06-17):** vá tài liệu phân rã FUNC theo bốn quyết định nghiệp vụ đã chốt giữa BRD PH4 v0.3 và **BRD PH4 v0.5** (xem [`BRD-TOSS-PH4-quan-ly-danh-muc-v0.5.md`](../../brd/BRD-TOSS-PH4-quan-ly-danh-muc-v0.5.md)):
>
...
| NG-1b | `BRD-TOSS-PH4-quan-ly-danh-muc-v0.5.md` (mã BR mới — dùng cho ánh xạ §4) | BR-401 … BR-469 (10 nhóm A–J) |
...
| Mã FUNC | Mô tả chức năng | BR cha | Nguồn |
|---|---|---|---|
| FUNC-424-01 | Quản lý năng lực hệ thống hạ cánh bằng thiết bị (ILS — Instrument Landing System) của tàu bay qua trường `ILS_EQUIPMENT` (vd giá trị `I` quan sát trong mẫu). | BR-424 | NG-6 cột `ILS_EQUIPMENT` |
| FUNC-424-02 | Quản lý năng lực tự động hạ cánh của tàu bay qua trường `AUTOLAND` (vd giá trị `Y` quan sát trong mẫu). | BR-424 | NG-6 cột `AUTOLAND` |
| FUNC-424-03 | Quản lý năng lực hệ thống truyền tin ACARS (Aircraft Communications Addressing and Reporting System) của tàu bay qua trường `ACARS` (vd giá trị `V` quan sát ở một số tàu A320 trong mẫu). | BR-424 | NG-6 cột `ACARS` |
| FUNC-424-04 | Quản lý các thiết bị đặc biệt khác của tàu bay qua trường `SPECIAL_EQUIPMENT` (danh mục mở, do nguồn không liệt kê cố định). | BR-424 | NG-6 cột `SPECIAL_EQUIPMENT` |
...
| Mã FUNC | Mô tả chức năng | BR cha | Nguồn |
|---|---|---|---|
| FUNC-426-01 | Quản lý trường `AP_RESTRICTION` (hạn chế sân bay khai thác) gắn với từng bản ghi tàu bay trong master tàu bay *(xem OID: SME-31)*. | BR-426 | NG-6 cột `AP_RESTRICTION`; NG-1 §7.4 BR-426 |
...
| FUNC-427-04 | Hợp nhất thông tin cấu hình ghế từ hai nguồn: FIMS có cột chi tiết `SEAT`, `C_CLASS`, `WY_CLASS`, `C_NC`, `Y_NY`, `CENTER_TANK`; Netline có cột `STD_VERSION` + 4 cột `STD_VERSION_ALT_n`. | BR-427 | NG-7; NG-6 |
...
| FUNC-427-06 | Ghi nhận lịch sử thay đổi của bản ghi golden record (kế thừa cơ chế versioning BR-403/FUNC-403-01..02) và lưu thông tin cập nhật cuối cùng từ Netline (`LAST_UPDATE`, `LAST_UPDATE_USER_ID`). | BR-427 | NG-6 cột `LAST_UPDATE`, `LAST_UPDATE_USER_ID`; NG-2 §II.4 |
...
*Phiên bản 0.4 — 2026-06-17 — BA Lead — Vá theo BRD PH4 v0.5 (A–D): (A) BR-431 PF Reports về PH3 — gỡ FUNC-408-05 Trend PF, thêm note ở §1.8 và §1.31; (B) BR-419 đồng bộ AMOS không tự quản lý bảo dưỡng — viết lại FUNC-409-02/03/04 ở §1.9; (C) Nhóm K BR-470…BR-473 không có FUNC ở v0.3 cần xóa, ghi chú tham chiếu PH2 v0.6 ở §2; (D) BR-425 thêm OID: SME-44 trong §1.25; chuẩn hóa toàn bộ cờ `[cần xác nhận]` về tham chiếu OID-TOSS-001 trong §3.1; thêm §4 ánh xạ BR cũ↔BR mới để truy vết hai chiều; cập nhật frontmatter (`version 0.3 → 0.4`, `date 2026-06-12 → 2026-06-17`, `parent_document → BRD-TOSS-PH4-quan-ly-danh-muc-v0.5.md`, `brd_version: v0.5`); tổng FUNC 142 → 141. Lịch sử các phiên bản trước (v0.1, v0.2, v0.3): xem `BA-VERSION-LOG.md`. Bản v0.3 lưu song song, chưa xóa.*
