...
> *(Cảnh báo chất lượng ASR: Transcript gốc có chất lượng nhận dạng giọng nói (ASR) trung bình — nhiều đoạn nhiễu, lặp từ, và thuật ngữ kỹ thuật bị biến dạng. Các thuật ngữ đã được hiệu chỉnh theo domain-knowledge: "mail/mèo/meo" → MEL, "AMOZ/AMBOT/HAMOS" → AMOS, "FOIE/e-FON/FOI/FOM" → e-FON [đã xác nhận: e-FON — Electronic Flight Operations Notice, BA Lead 17/06/2026], "OIP/OFP" → OFP, "NAIL/nail" → MEL item. Một số đoạn không khôi phục được — gắn cờ `[cần xác nhận]`.)*
...
e-FON (Electronic Flight Operations Notice) là hệ thống VNA dự kiến triển khai sau TOSS, chưa có hợp đồng triển khai. VNA và VTIT thống nhất: TOSS sẽ tích hợp AMOS một lần và cung cấp dữ liệu MEL cho e-FON khi hệ thống đó sẵn sàng — e-FON không cần tích hợp AMOS riêng.

Việc chi tiết hóa giao diện TOSS ↔ e-FON sẽ được thực hiện khi e-FON bước vào giai đoạn triển khai; hiện tại không cần đặc tả chi tiết hơn.

**Kết luận**

TOSS chịu trách nhiệm tích hợp AMOS để lấy toàn bộ dữ liệu MEL/CDL phục vụ nghiệp vụ khai thác. Khi e-FON được triển khai, TOSS sẽ chuyển dữ liệu sang e-FON theo yêu cầu. Chi tiết giao diện TOSS ↔ e-FON để sau khi có hợp đồng triển khai e-FON.
...
|---|---|---|
| **e-FON** (Electronic Flight Operations Notice) | ~~Chờ xác nhận~~ — **Đã xác nhận 17/06:** Thông báo Khai thác Bay Điện tử; hệ thống VNA dự kiến sau TOSS; nhận dữ liệu MEL từ TOSS. ASR ghi sai: "FOIE/FOI/FOM/FOEM". Glossary v0.16 đã cập nhật. | Dòng 3–6, 11, 17 |
