...
> **Cảnh báo chất lượng ASR:** Bản ghi gốc do hệ thống nhận dạng giọng nói tự động sinh ra nên còn nhiều lỗi chính tả thuật ngữ (NetLite/NetLite Off/Netop → NetLine/OPS++/NetOps; TOS/Top/Tóc/Tốt → TOSS; LSTP/LFTP/NFTP/FTP → SFTP (xác định qua bản ghi); VNL/VLA → VNA; FN/FF/FFM/SSHM/MVT/MVA/LDM còn lẫn lộn) và một số đoạn lặp dài bị bỏ qua. Báo cáo này đã hiệu đính theo nội dung thực tế và gắn cờ `[cần xác nhận]` tại các vị trí ASR chưa rõ.
...
- Bốn loại điện văn vận hành đi qua mạng SITA Text gồm điện movement MVT, điện MVA, điện divert (chuyển hướng) và điện LDM (load message tải) `[19062026 00:18:24]` `[19062026 00:18:48]`. Đối chiếu định dạng điện văn chuẩn của ngành (IATA Type B): MVT (Movement) và MVA (Movement Advice) là điện chuyển động tàu bay, gồm các điện Departure (định danh AD, giờ rời bến và cất cánh), Arrival (định danh AA, giờ hạ cánh và vào bến) và Delay (mã trễ chuyến); điện divert tương ứng điện DIV (Diversion) thông báo chuyển hướng hạ cánh ngoài kế hoạch; điện LDM là điện phân bổ tải. Chi tiết phần tử dữ liệu và ví dụ theo tài liệu OAG và sách trắng Type B Messaging của IATA.
- Điện FMM (Fuel Monitoring Message) là điện do TOSS phải tự sinh ra từ dữ liệu tổng hợp các nguồn khác để chuyển sang OPS++. Đại diện Viettel đã thắc mắc về định nghĩa điện này và được xác nhận đây là điện liên quan đến dữ liệu nhiên liệu, không phải điện hàng hóa hay điện hành khách, không liên quan kỹ thuật bảo dưỡng `[19062026 00:19:14]` `[19062026 00:20:07]` `[19062026 00:20:33]`. Bản ghi đọc thành "FFM" nhưng bảng "Thông tin tích hợp" do VNA tổng hợp ghi rõ là **FMM (Fuel Monitoring Message)**, hiện chưa có sẵn nên TOSS cần tự sinh.
...
**Yêu cầu.** Phương thức truyền dữ liệu chính được VNA đề xuất là giao thức truyền tệp an toàn (SFTP — Secure File Transfer Protocol). Bản ghi phiên âm cụm này thành nhiều dạng khác nhau, nhưng có hai vị trí đọc thẳng là "sftp" và cơ chế xác thực bằng Private Key đúng đặc trưng của SFTP nên xác định đây là SFTP. VNA sẽ chủ trì xây dựng máy chủ SFTP đặt trên hạ tầng của VNA, cấp quyền truy cập cho phía Lufthansa Systems để bên đó kết nối vào lấy và đẩy dữ liệu `[19062026 00:03:33]` `[19062026 00:04:00]` `[19062026 00:06:36]` `[19062026 00:07:06]`.
...
**Kết luận.** Hai bên thống nhất phương thức tích hợp chính là SFTP. VNA chịu trách nhiệm xây dựng máy chủ SFTP và cấu trúc thư mục theo kiến nghị NetOps gồm ba thư mục con Temp, Pickup, Archive cho mỗi đầu dữ liệu. Phương thức kết nối là VPN trên dải IP nội bộ `[19062026 00:31:35]` `[19062026 00:32:02]`.
...
|---|---|---|---|
| 1 | Danh mục đầy đủ và mã hóa từng luồng trong 17 luồng inbound và 2 luồng outbound. Cần có bảng đầy đủ kèm tên hệ thống nguồn, loại dữ liệu, mẫu điện đại diện. | Buổi này chỉ liệt kê các nhóm chính, chưa đủ cho việc triển khai. | Yêu cầu bạn An phía VNA cung cấp bảng tổng hợp Excel hoàn chỉnh trước thứ ba 24/06. |
| 2 | Định dạng dữ liệu Lufthansa Systems yêu cầu cho từng luồng (kể cả mẫu điện FMM mà TOSS phải tự sinh). | Cần định dạng chuẩn để TOSS sinh đúng tệp tin và truyền qua SFTP. | VNA làm việc với Lufthansa Systems, gửi tài liệu định dạng cho Viettel xem trước. |
| 3 | Cấu trúc thư mục chuẩn (folder structure) cho từng đầu dữ liệu kèm quy tắc đặt tên thư mục, quy tắc đặt tên tệp, mã hóa ký tự. | Là tiền đề cấu hình máy chủ SFTP. | Lấy tài liệu kiến nghị của NetOps. |
| 4 | Mốc thời gian gửi và đồng bộ dữ liệu cho từng luồng inbound và outbound, kèm thời gian lưu trữ trên SFTP và thời điểm xóa tệp. | Quyết định kiến trúc lưu trữ và lịch chạy tác vụ của TOSS. | VNA làm việc tiếp với Lufthansa Systems. |
| 5 | Định nghĩa hệ thống Schedule Manager `[cần xác nhận tên đầy đủ]` sinh tệp lịch bay ba lần một ngày. | Buổi họp đề cập nhưng tên chưa rõ ràng trong bản ghi. | Hỏi xác nhận với bạn An. |
| 6 | Phương án outbound cuối cùng từ OPS++ qua ISB hay phối hợp ISB cùng data duplicate. | VNA chưa chốt phương án. Ảnh hưởng kiến trúc thu nạp dữ liệu phía TOSS. | Đề nghị VNA chốt sau khi làm rõ với Lufthansa Systems. |
| 7 | Năm chính xác của mốc cắt chuyển hoàn toàn từ NetLine cũ sang OPS++ (bản ghi nêu “tháng 3”). | Bản ghi không nêu rõ năm; cần xác nhận để lập kế hoạch chuyển tiếp. | Hỏi xác nhận với anh Nam phía VNA. |
| 8 | Bốn nhóm thông tin hành khách lấy từ Amadeus PSS được đề cập tới các số “10, 11, 7, 8, 9” trong sample. Cần đối chiếu lại số thứ tự chính xác trong bảng làm việc chung. | Bản ghi gốc không thống nhất giữa “số 11 và số 789” và “số 10 với cả số 5”. | Đối chiếu với bảng làm việc của bạn An. |
| 9 | Trách nhiệm cuối cùng về phần TOSS xử lý dữ liệu nội bộ sau khi đã nhận từ OPS++. Phía VNA nói phần này “không thuộc scope dự án tích hợp NetLine” nhưng vẫn là việc TOSS phải làm. | Cần làm rõ để tránh xung đột về phạm vi giữa các gói dự án. | Đưa vào sổ OID và nêu trong cuộc họp tổng kết giai đoạn. |

> **Cập nhật sau buổi (23/06/2026).** Bảng "Thông tin tích hợp" do VNA tổng hợp đã được kết nối trực tiếp (Google Sheet live). Bảng cung cấp danh mục đầy đủ **24 luồng** inbound/outbound kèm nguồn, phương thức (SFTP/Email), tần suất, hình thức đồng bộ và trạng thái triển khai (Đã có / Missing) — giải quyết phần lớn mục 1 và bổ sung dữ liệu cho mục 4. Một số ô trong bảng vẫn ghi "Làm rõ" (thời điểm gửi, tần suất của nhiều luồng), tương ứng các điểm còn để ngỏ. Các mục 2, 3, 5–9 vẫn cần làm rõ thêm.
...
| 6 | **SFTP (Secure File Transfer Protocol)** | Giao thức truyền tệp an toàn | Phương thức truyền dữ liệu chính giữa TOSS và OPS++. Bản ghi phiên âm thành nhiều dạng (LSTP, LFTP, NFTP, FTP) nhưng xác định là SFTP do có hai vị trí đọc thẳng "sftp" và dùng xác thực Private Key. | `[19062026 00:03:33]` `[19062026 00:04:00]` |
