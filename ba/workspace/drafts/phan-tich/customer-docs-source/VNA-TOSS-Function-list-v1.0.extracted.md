---
source_gdrive: "https://docs.google.com/spreadsheets/d/15icZM004vwMt0YvUbrLpj-2Z40T0QXsE"
source_name: "VNA.TOSS_Function_list_v1.0.xlsx"
document_type: "Google Drive → MD (read-only)"
source_version: "404"
source_modified: "2026-06-16T04:17:40.028Z"
last_modifying_user: "chuyenly2003"
pulled: "2026-06-16"
status: "Raw pull — chưa biên tập"
---

> **Nguồn (Google Drive, live):** VNA.TOSS_Function_list_v1.0.xlsx — https://drive.google.com/file/d/15icZM004vwMt0YvUbrLpj-2Z40T0QXsE  
> Pull 2026-06-16 (version 404, sửa 2026-06-16T04:17:40.028Z bởi chuyenly2003).

## Tổng Quan
| Unnamed: 0 | Unnamed: 1 | Unnamed: 2 | Unnamed: 3 | Unnamed: 4 | Unnamed: 5 | Unnamed: 6 | Unnamed: 7 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| NaN | STT | Module | Link Design | NaN | STT | System | Email |
| NaN | 1 | NaN | NaN | NaN | 1 | NaN | NaN |
| NaN | 2 | NaN | NaN | NaN | 2 | NaN | NaN |
| NaN | 3 | NaN | NaN | NaN | 3 | NaN | NaN |
| NaN | 4 | NaN | NaN | NaN | 4 | NaN | NaN |
| NaN | 5 | NaN | NaN | NaN | 5 | NaN | NaN |
| NaN | 6 | NaN | NaN | NaN | 6 | NaN | NaN |
| NaN | 7 | NaN | NaN | NaN | 7 | NaN | NaN |
| NaN | 8 | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | 9 | NaN | NaN | NaN | NaN | NaN | NaN |

## Function list
| TOSS - Total Operations Steering System Function list TOSS | Unnamed: 1 | Unnamed: 2 | Unnamed: 3 | Unnamed: 4 | Unnamed: 5 |
| --- | --- | --- | --- | --- | --- |
| STT | Module | Chức năng | Chức năng con lv1 | Mô tả | Ghi chú |
| 1 | Live Operations | NaN | NaN | NaN | NaN |
| NaN | NaN | Nhập thông tin hỏng hóc kỹ thuật | NaN | NaN | NaN |
| NaN | NaN | Nhập thông tin dịch vụ | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| 2 | Flight Dispatch | NaN | NaN | NaN | NaN |
| NaN | Flight monitoring | Màn hình giám sát danh sách tổng quan | NaN | Tham chiếu kịch bản sheet màn hình Flight monitoring | NaN |
| NaN | NaN | Lọc trên màn hình | NaN | NaN | NaN |
| NaN | NaN | Release OFP | NaN | NaN | NaN |
| NaN | NaN | Un release OFP | NaN | NaN | NaN |
| NaN | NaN | NaN | Xem chi tiết chuyến bay | Mở cửa sổ mới | NaN |
| NaN | NaN | NaN | Upload OFP trong details | NaN | NaN |
| NaN | NaN | NaN | Các tab thông tin trong xem chi tiết chuyến bay | NaN | NaN |
| NaN | Upload common Document | Màn hình danh sách tài liệu chung chuyến bay | NaN | NaN | NaN |
| NaN | NaN | Xoá tài liệu chung chuyến bay | NaN | NaN | NaN |
| NaN | NaN | Upload tài liệu chung chuyến bay | NaN | Cho chọn quãng thời gian hiệu lực có thể đưa lên MO theo ETD. | NaN |
| 3 | Flight Load Control | NaN | NaN | NaN | NaN |
| NaN | Tab1 | Màn hình Danh sách Chuyến bay kèm tài liệu từng chuyến | NaN | Phân quyền theo chức năng thì sẽ hiển thị được các cột tương ứng thưo từng cụm tài liệu\nCó customize bảng biểu\nCó highlight, status để báo cảnh báo\nThời gian upload tài liệu lần cuối,... | Tài liệu cần noti tới MO để sync được ngay lập tức |
| NaN | NaN | Upload tài liệu từng chuyến | NaN | Vẫn có option ẩn hiện lên MO | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Tab2 | Màn hình danh sách upload nhiều tài liệu cho nhiều chuyến bay | NaN | NaN | NaN |
| NaN | NaN | Upload nhiều loại tài liệu cho nhiều chuyến bay | NaN | Có cơ chế nhận diện qua tên file hoặc content của file. | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| 4 | Station Manager | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| 5 | Report | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| 6 | Data Maintenance | NaN | NaN | NaN | NaN |
| NaN | Quản lý danh mục tàu bay | Quản lý thông tin tàu bay | NaN | Quản lý được thời hạn vào ra\nSở hữu\nTàu thuê (thuê khô đội vận hành tàu là của VNA), thuê ướt (thuê toàn bộ chuyến bay, tiếp viên khác đó, pilot hãng đó)\n | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | Quản lý MEL, CDL, defect | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | Quản lý Performance Factor | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Quản lý danh mục Sân bay | NaN | NaN | Có khai báo taxi time tiêu chuẩn cho từng loại tàu đối với từng sân bay, có thời gian hiệu lực, có gợi ý taxi time để user ghi nhận. | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Quản lý danh mục Chặng bay | Chặng bay | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | Tankering | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Quản lý giá nhiên liệu | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Quản lý danh mục ULD | Danh sách ULD | NaN | Hiển thị danh sách tổng hợp toàn bộ ULD trên hệ thống | NaN |
| NaN | NaN | Tìm kiếm | NaN | Cho phép người dùng tìm kiếm dữ liệu ULD theo Mã , Loại ULD (AKE, PMC, PAG...), Trạng thái hoạt động, Khoảng Trọng lượng rỗng, hoặc Vị trí sân bay hiện tại. | NaN |
| NaN | NaN | Kết xuất | NaN | Cho phép nguời dùng kết xuất toàn bộ danh sách hoặc kết xuất theo bộ lọc dữ liệu ULD ra file định dạng .xlsx hoặc .csv | NaN |
| NaN | NaN | Thêm mới ULD | NaN | Cho phép nguời dùng khởi tạo 1 ULD mới được hãng mua hoặc thuê bao gồm các trường như. Mã ULD, Loại , trọng lượng rỗng, trọng lượng tối đa, Thể tích, Trạng thái, Vị trí sân bay hiện tại | NaN |
| NaN | NaN | Sửa ULD | NaN | Cho phép người dùng cập nhật hoặc chỉnh sửa các thông số của một ULD đang tồn tại trên hệ thống | NaN |
| NaN | NaN | Xóa ULD | NaN | Cho phép người dùng loại bỏ hồ sơ của một ULD khỏi hệ thống (thường áp dụng khi nhập sai hoặc thiết bị đã được thanh lý). | NaN |
| NaN | NaN | Xem chi tiết ULD | NaN | Cho phép người dùng xem lại toàn bộ hồ sơ "lý lịch" của một ULD ở chế độ chỉ đọc (Read-only), bao gồm các trường như: Mã ULD, Loại, Trọng lượng rỗng, Trọng lượng tối đa, Thể tích, Trạng thái, Vị trí hiện tại, Người tạo, Ngày tạo, Các chuyến bay gần nhất đã sử dụng thiết bị này | NaN |
| NaN | NaN | Xem lịch sử cập nhật ULD | NaN | Cho phép người dùng tra cứu toàn bộ dấu vết thay đổi dữ liệu (Audit Trail) của một ULD bao gồm các trường như: Thời gian thay đổi, Người thực hiện, Thao tác, và Chi tiết thay đổi dữ liệu (Giá trị cũ -> Giá trị mới) | NaN |
| NaN | Quản lý danh mục FIR | Danh sách FIR | NaN | NaN | NaN |
| NaN | NaN | Tìm kiếm | NaN | NaN | NaN |
| NaN | NaN | Kết xuất | NaN | NaN | NaN |
| NaN | NaN | Thêm mới FIR | NaN | NaN | NaN |
| NaN | NaN | Sửa FIR | NaN | NaN | NaN |
| NaN | NaN | Xóa FIR | NaN | NaN | NaN |
| NaN | NaN | Xem chi tiết FIR | NaN | NaN | NaN |
| NaN | Quản lý danh mục phi công | Danh sách phi công | NaN | NaN | NaN |
| NaN | NaN | Xuất danh sách phi công | NaN | NaN | NaN |
| NaN | NaN | Xem chi tiết phi công | NaN | NaN | NaN |
| NaN | NaN | Thêm mới phi công | NaN | NaN | NaN |
| NaN | NaN | Sửa thông tin phi công | NaN | NaN | NaN |
| NaN | Quản lý danh mục Tiếp viên | Danh sách tiếp viên | NaN | NaN | Đồng bộ từ aves |
| NaN | NaN | Xuất danh sách tiếp viên | NaN | NaN | NaN |
| NaN | NaN | Xem chi tiết tiếp viên | NaN | NaN | NaN |
| NaN | NaN | Thêm mới tiếp viên | NaN | NaN | NaN |
| NaN | NaN | Sửa thông tin tiếp viên | NaN | NaN | NaN |
| NaN | Quản lý danh mục Quốc gia | Danh sách quốc gia | NaN | NaN | NaN |
| NaN | NaN | Thêm mới quốc gia | NaN | NaN | NaN |
| NaN | NaN | Sửa thông tin quốc gia | NaN | NaN | NaN |
| NaN | NaN | Xóa quốc gia | NaN | NaN | NaN |
| NaN | Quản lý danh mục Carrier | Danh sách Carrier | NaN | NaN | NaN |
| NaN | NaN | Thêm mới carrier | NaN | NaN | NaN |
| NaN | NaN | Sửa carrier | NaN | NaN | NaN |
| NaN | NaN | Xóa carrier | NaN | NaN | NaN |
| NaN | Quản lý danh mục cơ quan đơn vị | Danh sách cơ quan đơn vị | NaN | NaN | NaN |
| NaN | NaN | Thêm mới cơ quan đơn vị | NaN | NaN | NaN |
| NaN | NaN | Sửa thông tin cơ quan đơn vị | NaN | NaN | NaN |
| NaN | NaN | Xóa cơ quan đơn vị | NaN | NaN | NaN |
| NaN | Quản lý danh mục DOW | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| 7 | Data Source Monitoring | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN |
| 8 | System Admin | NaN | NaN | NaN | NaN |
| NaN | Quản lý tham số trần và sàn ZFW | NaN | NaN | Cho phép khai báo để làm cảnh báo khi ZFW thay đổi từ CLC với kế hoạch bay nếu giảm hoặc tăng khác ngưỡng.\n Theo loại tàu bay\nTheo khoảng giờ bay (flight time) | NaN |

## Màn hình Flight Dispatch
| Menu bar | Unnamed: 1 | Unnamed: 2 | Unnamed: 3 | Header | Unnamed: 5 | Unnamed: 6 | Unnamed: 7 | Unnamed: 8 | Unnamed: 9 | Unnamed: 10 | Unnamed: 11 | Unnamed: 12 | Unnamed: 13 | Unnamed: 14 | Unnamed: 15 | Unnamed: 16 | Unnamed: 17 | Unnamed: 18 | Unnamed: 19 | Unnamed: 20 | Unnamed: 21 | Unnamed: 22 | Unnamed: 23 | Unnamed: 24 | Unnamed: 25 | Unnamed: 26 | Unnamed: 27 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| NaN | NaN | NaN | Filter 2 | Filter 1 | Filter 3 | Filter 4 | NaN | Filter 5 | Filter 6 | Filter 7 | Filter 8 | Filter 9 | NaN | Filter 10 | Filter 11 | Filter 12 | Filter 13 | Filter 14 | Filter 15 | Filter 16 | Filter 17 | Filter 18 | Filter 19 | Filter 20 | Filter | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| Flight monitoring | --> | NaN | Bảng danh sách chuyến bay | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| Upload common documents | NaN | DSP Release | REG | FLTNO | DEP | ARR | Date | ETD | ETA | Flight Type | Status | OFP DSP | Pilot release | EPLD | EST DOW | BLOCK FUEL | PILOT EXTRA | NOTAM | WX | ZFW-DOW | MEL/CDL | ATC | TO/LD (MTOW, MLDW) | Missing Document | Taxi APU | Crew change | VIP |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | - Check flight Type: (Thay đổi type của chuyến bay)\nLưu lịch sử thay đổi của loại chuyến bay.\nVIP- thường\nThường-Ferry\nngược lại | Trạng thái CB (đang ở phase nào), tạm thời: \nGRD: Ground, đang PVMĐ\nBRD: Boarding\nOUT: Taxi out\nENR: Enroute\nIN: Taxi in\nARR: Arrival (đã kết thúc)\nKhi chuyển trạng thái thì nhấp nháy mấy lần rồi dừng | Hiển thị OFP number và OFP Rev cuối dạng 2/0/1 R2.\n\nNếu Rev cuối đó đã dispatch release thì XANH.\n\nNếu Rev cuối chưa release mà có rev trước đó release thì VÀNG.\n\nNếu Rev cuối chưa release mà trước đó chưa có rev nào được dsp relesae thì ĐỎ | NaN | Hiển thị giá trị PAYLOAD EST trên OFP.\nCảnh báo tại đây nếu Payload CLC nhập sai khác quá giới  hạn | Hiển thị giá trị DOW EST trên OFP.\nCảnh báo tại đây nếu DOW CLC nhập sai khác quá giới hạn | OFP BLOCK FUEL | Nếu CB chưa có Flight Release thì bỏ trống.\nCảnh báo màu nếu 30 phút trước EDT mà Pilot chưa release,\n\nNếu đã có Pilot Release, hiển thị giá trị Pilot Release - OFP Block Fuel của đúng rev OFP được release bởi Pilot | "- Check NOTAM (trên trang của quản lý bay hoặc LIDO)\nKiểm tra NOTAM mới phát sinh\nĐánh giá ảnh hưởng (So với đường bay có ảnh hưởng hay không như hoạt động quân sự)\nCác loại NOTAM quan trọng như Đóng cửa sân bay, đường bay, quân sự.\nCheck cách cứu hoả RFFS (Sân có public cách cứu hoả rồi, có NOTAM là cảnh báo rồi)" | - Check thời tiết WX so với Minima của từng sân bay (đối với sân cất-hạ cánh), nguồn thời tiết (nội địa- trang web quản lý bay, Quốc tế lấy theo LIDO)\nMinima của từng sân bay sẽ có tiêu chuẩn \nQuan tâm\n+Tầm nhìn\n+Trần mây\n+Mưa giông (TSRA, TSRA+)\n+ Airport chart (lấy từ Quản lý bay, Weather new)\nCăn cứ vào bản tin META (30p/lần) | - Check ZFW (đổi tải trong FON), 60p quốc nội, 90p quốc tế.\nCăn cứ số liệu được nhập trên CLC estimate với OFP hiện tại, giới hạn lệch bao nhiêu thì cần khai báo, có giới hạn trần và sàn, khai báo trong systemadmin\n- Check DOW (trong AHM) (Drive operating weight) | "- Cảnh báo MEL/CDL (tích hợp AMOS) (Master MEL)\n\nLỗi kỹ thuật mà cần mang thêm dầu trong các MEL\nẢnh hưởng mực bay\n" | Cảnh báo ATC (nội dung như MO) | Cảnh báo est TO/LD (MTOW, MLDW) trong OFP\nSân bay khai thác (FOE) phần mềm tính performance, điều kiện taf meta để tính performance và so với kế hoạch. | Cảnh báo chuyến bay thiếu tài liệu (OFP, NOTAM, WX) | Cảnh báo Taxi, APU so với (FON)\nLệch taxi time\n(Danh mục sân bay sẽ cần khai báo taxitime tiêu chuẩn) | Cảnh báo tổ bay chưa download tài liệu | Cảnh báo chuyến VIP mà ATC, OFP chưa có Status head, trước chuyến bay. |
| NaN | Dữ liệu mẫu | Check box | AZYX (không có VN)\nTàu XU giữ nguyên | Đầy đủ | NaN | NaN | dd | 16:00:00 | 16:00:00 | Theo flight type | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| Các cảnh báo sẽ được phân màu theo rules đã thống nhất và khi hover chuột vào sẽ hiển thị tool tips chi tiết của cảnh báo theo từng ô, Khi bấm vào chi tiết chuyến bay để xem full. | Color | Active khi ofp đã tới thời điểm cần release\n\n\nKhi nhấn vào sẽ release ofp.\n\nĐiều kiện active là bóc tách OFP xem user DSP với số liscence nào tương ứng với thông tin trên DSP lisence trong thông tin user | Đỏ - OFP DSP release khác Netline\nKhông màu - Chưa có OFP\nXanh - Sau đỏ, OFP = Netline\nVàng - OFP giống Netline và DSP chưa release | - Surfix D,Z để vàng\n- Mất D, Z Báo đỏ\n- Ops++ hoặc netline đổi status ARR Hoặc có ACARS IN thì back về thường\n- Thường là các trường hợp còn lại\n | NaN | Chuyến Divert\nVàng - Khi thay đổi \n(ARR kế hoạch và thực tế khác nhau trong Ops++) | Theo màu của cột Flight Num | - Vàng - 15 tới 30P so với EDT trước đó trên netline\n- Xanh - Gửi điện change và lấy ETD mới so sánh\n- Đỏ - netline kéo delayed trên 30p\n- Xanh - khi ATC vs OFP phải release khớp\n | NaN | - Vàng: đã ATC và OFP ra nhưng chưa release\n- Đỏ: Thường -> Ferry và ngược lại\n- Đỏ: Thường -> VIP và ngược lại\n- Xanh: Status head ATC và OFP đã release khớp áp dụng chuyến thường sang VIP và ngược lại.\nVới thường sang ferry và ngược lại thì dựa vào số pax trong OFP. | Chờ xem xét tích hợp | - Chưa có OFP:\n (-) nếu chưa tới giờ\n (!) Vàng nếu sắp tới giờ (210 phút với quốc nội, 270 phút với quốc tế)\n (!) Đỏ nếu sắp tới giờ (75 phút với quốc nội, 95 phút với quốc tế)\n\n- Đã có OFP, DSP chưa release:\nHiển thị Phiên bản OFP\nMàu vàng từ (75->60 quốc nội), 90->75 quốc tế)\nMàu đỏ từ 60 quốc nội, 75 quốc tế\n\n- Đã có OFP, DSP đã release:\n Hiển thị phiên bản OFP\nMàu xanh\n | Chưa confirm\n- gạch trắng\n\nĐã confirm\n- Hiển thị phiên bản OFP R01\n\n- Màu xanh đã release OFP cuối mà DSP đã Release\n- Màu vàng đã release OFP trước OFP cuối OFP release\n\n- Màu đỏ Reject. | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Hover details | NaN | Reg cũ --> Reg mới\nGiờ đổi trên netline | - | - | Thông tin ARR Divert và giờ thực tế\n\nARR kế hoạch -> ARR thực tế | NaN | Giờ cũ -> giờ mới | NaN | Hiển thị lịch sử thay đổi code nào sang code nào và thời điểm thay đổi | NaN | Thời gian upload, Tên DSP,  thời gian release. | Thời gian release. | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | Details | NaN | Lịch sử đổi tàu: Các mốc thời gian và tàu tương ứng | Lịch sử thay đổi số hiệu chuyến bay | NaN | NaN | NaN | Lịch sử netline kéo các mốc giờ | NaN | Lịch sử đổi code | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | Phân quyền theo carrier | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| Bấm vào details sẽ nhảy sang tab mới chi tiết chuyến bay, bấm ở ô nào thì sẽ hiển thị tab tương ứng trong tab đó. | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| Lưu cấu hình bảng biểu và search | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |
| Màu:\nKhông màu - Bình thường\nĐỏ - cần giải quyết\nVàng - lưu ý\nXanh lá - Đã xử lý | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN | NaN |