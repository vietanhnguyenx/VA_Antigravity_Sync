...
| **AOG/MNT Schedule** | Lịch bảo dưỡng tàu bay | Màn hình danh sách lịch bảo dưỡng tàu bay, lấy nguồn cơ bản từ Netline (legtype = Z, FN_Carrier thuộc danh mục mã bảo dưỡng), chi tiết hạng mục công việc lấy từ AMOS khi xem chi tiết. TOSS hiển thị, không tạo lịch. (nguồn: KS 17/06/2026.) |
| **AOG/MNT Code** | Danh mục mã bảo dưỡng | Bảng danh mục mã và nhóm bảo dưỡng (thêm/sửa/xoá/lọc), phân theo 5 nhóm cố định (AOC, định kỳ, line check kỹ thuật, bảo quản, standby), dùng để lọc lịch bảo dưỡng. (nguồn: KS 17/06/2026.) |
...
| **FOE** | (giữ nguyên — tên đơn vị nội bộ VNA) | Đơn vị phát triển đường bay, lập kế hoạch bay chi tiết (detailed plan: route, flight time, aircraft, load), nhận tài liệu khai thác sau chuyến. Các nhóm trực thuộc xuất hiện trong nguồn: **OPS team, EFB Group, Aircraft Performance Group (PER)** (nguồn: FDOP §3.3, §3.5). *(Cần xác nhận tên đầy đủ với VNA.)* *(Lưu ý: buổi KS 17/06 gọi bộ phận của chị Nga phụ trách MEL/Performance là "FOE — Flight Ops Engineering"; tên mở rộng này CHƯA xác nhận và có thể trùng/khác với FOEM (Flight Ops Engineering, đơn vị cấp Bảng DOW). Cần SME VNA làm rõ FOE ≟ FOEM. [cần xác nhận])* |
...
| **FN_Carrier** | (giữ nguyên — trường Netline) | Trường mã hãng/chuyến trong Netline, kết hợp với legtype = Z để xác định các chuyến bảo dưỡng khi đồng bộ lịch bảo dưỡng về TOSS. Mã FN_Carrier mới chưa khai báo trong AOG/MNT Code có thể gây bỏ sót kỳ bảo dưỡng (xem OID KS-68). (nguồn: KS 17/06/2026.) |
...
| **Max Ramp Weight (MRW)** | Trọng lượng tối đa khi lăn | Trọng lượng tối đa cho phép của tàu bay khi rời bãi đỗ (trước khi tiêu hao nhiên liệu lăn), thuộc nhóm thuộc tính khai thác tàu bay do TOSS quản lý. (nguồn: KS 17/06/2026.) |
...
| **MVA** *(Multi-format Variant Address — luồng ACARS song song)* | Luồng điện ACARS song song MVA | Địa chỉ đích `QU HDQONVN` nhận phiên bản compact của cùng sự kiện OOOI, gửi song song với điện thường (`QU HANASVN`). Format ngắn gọn hơn: `AD<time> <dest>` (OUT/OFF), `AA<on_time>/<in_time>` (ON/IN), `SI FOB <fuel>/`. VNA **ưu tiên lấy OOOI time từ MVA trước** điện thường. Xuất hiện trong cả [A80](#a), M11/M13/M14, [DEP](#d)/[ARR](#a). *(Đơn vị FOB trong MVA: chờ xác nhận — OID SME-45.)* **Phân biệt với MVA (SITA — Movement Advice) ở mục dưới** — ngữ cảnh ACARS, khác hệ. (nguồn: tong-hop-dien-acars.extracted.md hàng 23/30/39/43). |
| **MVT** *(SITA — Aircraft Movement Message)* | Điện chuyển động tàu bay | Điện văn SITA Type B báo chuyển động chuyến bay qua mạng SITA Text, gồm điện Departure (định danh `AD` kèm giờ rời bến và cất cánh), Arrival (`AA` kèm giờ hạ cánh và vào bến), Delay (mã trễ chuyến); giờ theo UTC, phần tử Bắt buộc/Có điều kiện/Tùy chọn. Trong tích hợp TOSS↔OPS++ là luồng inbound qua SITA, phương thức SFTP/Email. (nguồn: KS 19/06/2026; bảng Thông tin tích hợp VNA; OAG MVT/MVA/DIV; IATA Type B Whitepaper.) |
| **MVA** *(SITA — Movement Advice)* | Điện tư vấn chuyển động | Biến thể của điện chuyển động [MVT](#m) trong nhóm điện vận hành SITA Text, đi cùng MVT/DIV/LDM trong tích hợp TOSS↔OPS++ (luồng inbound, SFTP). **Phân biệt với MVA (ACARS — Multi-format Variant Address) ở mục trên** — khác hệ. (nguồn: KS 19/06/2026; bảng Thông tin tích hợp VNA; OAG.) |
| **DIV — điện divert** *(SITA — Diversion Message)* | Điện chuyển hướng | Điện văn SITA Text thông báo chuyến bay chuyển hướng hoặc hạ cánh ngoài kế hoạch, gửi tới trung tâm điều hành và các trạm dọc đường. Trong tích hợp TOSS↔OPS++ là luồng inbound qua SITA (SFTP/Email). Transcript 19/06 gọi "điện divert". (nguồn: KS 19/06/2026; bảng Thông tin tích hợp VNA; OAG MVT/MVA/DIV; IATA Type B.) |
| **LDM** *(Load Distribution Message)* | Điện phân bổ tải | Điện văn tải (nằm trong loadsheet) trao đổi qua SITA Text; trong tích hợp TOSS↔OPS++ là luồng inbound (SFTP/Email), trạng thái hiện "Missing" trong bảng Thông tin tích hợp VNA. Định dạng chuẩn thuộc IATA AHM (nhóm điện tải) — chưa có tài liệu nguồn trong domain-knowledge. (nguồn: KS 19/06/2026; bảng Thông tin tích hợp VNA.) |
...
| **Valid From – Valid To** | Khoảng thời gian hiệu lực | Cặp thời điểm hiệu lực của một bản ghi (tàu bay, hệ số nhiên liệu...). Với danh mục tàu bay đồng bộ từ OPS++, TOSS lấy cặp giá trị mới nhất hiện hành trên Netline. (nguồn: KS 17/06/2026.) |
