...
> **Few-shot example for `business-analyst` agent.** This file shows what a fully-completed BRD looks like in target Vietnamese style. The example body is in Vietnamese intentionally — it is the style the agent must produce. Mirrored at `.claude/human/examples/ba/example-BRD-finished.md`.
