...
Suy luận nội bộ bằng tiếng Anh. Từ khóa Mermaid (`flowchart`, `-->`, `subgraph`) giữ nguyên vì là cú pháp. **Toàn bộ nhãn nút, văn bản quyết định, tên làn, và chú giải bằng tiếng Việt**, đồng bộ với [`.claude/glossary/ba-terms-vi-en.md`](../glossary/ba-terms-vi-en.md).

Mã định danh nút (ví dụ `A`, `B1`) giữ ký tự alphanumeric — chúng là cú pháp, không hiển thị cho người dùng.

## Quy trình

1. **Làm rõ quy trình:** điểm bắt đầu, điểm kết thúc, tác nhân, điểm quyết định. Nếu thiếu thông tin, dùng `AskUserQuestion` (tối đa 4 câu).
2. **Chọn loại sơ đồ:**
...
3. **Soạn mã Mermaid:** giữ ≤ 15 nút mỗi sơ đồ; tách thành sub-process nếu lớn hơn.
4. **Thêm chú giải tiếng Việt** giải thích các ký hiệu sử dụng.
5. **Kiểm tra:** hình dung sơ đồ trong đầu; chiều mũi tên đúng, mọi nhánh đều dẫn đến nút kết thúc.
